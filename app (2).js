...
const orgStructureData = {
  ...
  pengurusInti: [
    ...
    { id: "roro-pengurus", name: "Roro Riwayatiningsih, S.Pd", role: "Sekretaris Pengurus Inti", bio: "Tenaga administrasi yang menyusun agenda latihan, registrasi anggota, dan dokumentasi rapat.", dedication: "15 Tahun Pengabdian", contribution: "Pengarsipan arsip sejarah, manajemen korespondensi publik." },
...
function initOrgTreeChart() {
  ...
  // Build tree chart layers: Penanggung Jawab -> Pengurus Inti
  let treeHTML = `
    ...
    matches.slice(0, 5).forEach(m => {
      ...
      btn.addEventListener("click", () => {
        ...
        if (m.link === "#profiles") {
          ...
          const matchProfile = orgStructureData.penanggungJawab.concat(orgStructureData.pengurusInti).find(p => p.name === m.label);
          if (matchProfile) {
